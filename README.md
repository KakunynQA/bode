...
>
> **v2.4.0 — TUI landing screen.** The shell now opens with a responsive landing screen (ASCII goat art with full/compact/text fallback, multiline composer, command palette via Ctrl+P, leader-key shortcuts via Ctrl+X, status bar footer). `bode` with no args launches the TUI; explicit subcommands (`bode setup`, `bode start KD-1`, etc.) still work as one-shot invocations outside the TUI.

---

## Quick Start (TUI)

Running `bode` with no arguments opens the Ink-powered TUI shell (requires a TTY):
...
# Optional: absolute path to PROJECT_CONTEXT.md generated by
# `bode setup-project` investigation. Set automatically by the wizard.
...
- **Preflight** checks `workdir` and `repos[].workdir` are readable. If not, the phase aborts before spending tokens. (`context_paths` deprecated in v1.3.0.)
