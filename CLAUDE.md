...
- Stack: TypeScript strict, Bun, no DB (filesystem `~/.bode/`), MCP Atlassian for Jira, child_process for AI CLIs
- Run dev: `bun run dev <subcommand>`
- Validate: `bun run check && bun run lint && bun run test && bun run build`
- Never: `any`, direct Jira REST, direct git CLI outside `adapters/vcs/`, direct token reads outside keychain code
