...
**Bode** is a local CLI (v0.26.0) that orchestrates AI coding work through configurable phases (planning, implementation, review, PR creation), driving native AI CLIs interactively (terminal handoff). Since v0.18.0, bode does NOT shell out to `git` — the AI handles all git operations in its session. Bode reads handoff files (`<phase>.md`, `branch.txt`, `pr.txt`) to track state. Runs are protected by per-task lockfiles and meta.json writes are atomic (v0.19.0). v0.20.0 added `bode doctor`, `.bode.yml` repo-local config, and env auto-detect foundation. v0.21.0 made **Jira optional** via the new `LocalTrackerAdapter` (`.bode/tasks/<key>.md`). v0.22.0 added the **`bode <query>` fast path**. v0.23.0 closed **zero-config first run**. v0.24.0 added **GitHub Issues tracker**. v0.25.0 renamed `IssueTrackerStrategy` methods to provider-neutral names (`fetchTask`, `postComment`, `setStatus`, `addTag`, `removeTag`, `listStatuses`); old Jira-flavored names stay until v0.30.0 as `@deprecated` delegates. **Internal callers must use the new names.** v0.26.0 added **Linear, Notion, Trello** adapters + `tracker: plain-markdown` alias + `bode new "<summary>"` command — Wave 2 essentially closed. See `SPEC.md` for full requirements. See `CONVENTIONS.md` for code standards.
